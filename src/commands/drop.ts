...
import { parsePostgresUrl, merge, dropDb } from "../lib.js";
...
export default class Drop extends Command {
	...
	static override flags = {
		...
		url: Flags.string({
			char: "l",
			description:
				"URL of DB that will be dropped, e.g. postgres://username:password@localhost:5432/my_db",
			env: "DB_URL",
	...
	public async run(): Promise<void> {
		const parsed = await this.parse(Drop);
		...
		const finalParams = merge(rest, urlParams);
		...
		await dropDb(
			{ database, notExistsError, dropConnections },
